...
def ST_maybe(docs):
    """
    Generates a comprehensive legal ST_maybe for a lawyer based on the provided chargesheet details.

    Parameters:
    docs (str): The chargesheet details.

    Returns:
    A generator yielding the comprehensive legal ST_maybe in chunks.
    """

    system = """
   Based on the details provided in the current chargesheet, generate a comprehensive legal ST_maybe for the lawyer to apply in court. The ST_maybe should include the following elements:
   ...
   8. **Conclusion**
   - Summary of the overall ST_maybe and the desired outcome.

   Please ensure the ST_maybe is practical, aligned with legal principles, and tailored to the specifics of the chargesheet provided.
   """

    human = "{docs}"
    prompt = ChatPromptTemplate.from_messages([("system", system), ("human", human)])

    ST_maybe_chain = prompt | llm | StrOutputParser()
    for chunk in ST_maybe_chain.stream({"docs": docs}):
